...
def main():
	global FPSCLOCK, DISPLAYSURF, BASICFONT, L_SQUIR_IMG, L_SQUIR_IMG, GRASSIMAGES
...
def runGame():
	...
	while True: # MAIN GAME LOOP
		...
		# go through all objects to see if any need to be deleted
		for i in range(len(grassObjs) - 1, -1, -1):
			if isOutsideActivrArea(camerax, cameray, grassObjs[i]):
				del grassObjs[i]
		for i in range(len(squirrelObjs) - 1, -1, -1):
			if isOutsideActiveArea(camerax, cameray, squirrelObjs[i]):
				del squirrelObjs[i]
		
		# add more grass and squirrels if we don't have enough.
		...
		if (camerax + HALF_WINWIDTH) - playerCenterx > CAMERASLACK:
			...
			camerax = playerCenterx - CAMERASLACK - HALF_WINWIDTH
		if (cameray + HALF_WINWIDTH) - playerCentery > CAMERASLACK:
...
def getRandomVelocity():
	speed = random.random.randint(SQUIRRELMINSPEED, SQUIRRELMAXSPEED)
...
def isOutsideActiveArea(camerax, cameray, obj):
	...
	boundsRect = pygame.Rect(boundsLeftEdgem boundsTopEdge, WINWIDTH * 3, WINHEIGHT * 3)
